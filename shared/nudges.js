...
(function () {
  ...
  function removeCard(id, outcome, detail) {
    ...
    renderCards();
    if (outcome) NudgBus.emit("buddy", outcome, Object.assign({ id, rule: card.rule, mrn: card.mrn }, detail || {}));
    hidePeekFor(id);
  }
  ...
  function armR09(mrn) {
    ...
    dwellTimer = setTimeout(() => {
      ...
      commit({
        ...
        question: d.question,
        actions: [
          { id: "start_referral", label: "Start the referral", kind: "primary" },
  ...
  // R-12: a referral accepted from a nudge gets an owner, a deadline, and an escalation promise.
  function fireR12(mrn) {
    const p = byMrn.get(mrn);
    if (!p) return;
    commit({
      id: `r12:${mrn}`, rule: "R-12", type: "watch", tempo: "WATCH", mrn, name: p.name,
      headline: "Watching the referral: A reply is due by Friday, Jul 25.",
      bullets: [
        { t: "Owner: Dr. Rivera. Backup: Care coordination.", src: "R-12" },
      ...
      why: "You started a referral from a nudge · R-12 · silent until the deadline or a result",
    });
  }
  ...
  function runAction(card, actionId) {
    ...
    NudgBus.emit("buddy", "nudge_acted", { id: card.id, rule: card.rule, mrn: card.mrn, action: actionId });
    if (actionId === "open_rhythm") {
      NudgBus.emit("buddy", "nudg_cmd", { action: "ehr_open_doc", mrn: card.mrn, match: "" });
      NudgBuddy.toast("Opening the rhythm note in LegacyChart.");
      removeCard(card.id, null);
    } else if (actionId === "show_me") {
      NudgBus.emit("buddy", "nudg_cmd", { action: "ehr_open_doc", mrn: card.mrn, match: "" });
      removeCard(card.id, null);
    } else if (actionId === "start_referral") {
      NudgBuddy.toast("Referral drafted for your sign-off: Nothing is sent without you.");
      removeCard(card.id, null);
      fireR12(card.mrn);
  ...
  function cardHtml(card) {
    ...
    if (card.research) {
      body += `<div class="nbc-sect"><div class="nbc-sect-label">RESEARCH: ONE SPECIFIC REFERENCE</div>
        <ul class="nbc-evi"><li><span>${esc(card.research.title)}</span><span class="nbc-src">${esc(card.research.cite)}</span></li></ul></div>`;
    }
    ...
    return `<div class="nbc ${m.cls}" data-card-root="${esc(card.id)}">${chipRow(card)}<h4>${esc(card.headline)}</h4>${body}${actions}
      <div class="nbc-trace">Why this, why now: ${esc(card.why)} · 1st time shown</div></div>`;
  }
  ...
  function renderCards() {
    NudgBuddy.setNudgeCount(cards.size);
    if (panelCard) return; // panel view owns the container
    const el = NudgBuddy.cardsEl;
    if (!cards.size) {
      el.innerHTML = `<div class="nudg-cards-empty">Nothing needs your attention. You'll only hear from me when I can point at something specific in this patient's chart.</div>`;
      return;
    }
    el.innerHTML = [...cards.values()].slice().reverse().map(cardHtml).join("");
  }
  ...
  async function relay(path, body, ms) {
  ...
  function panelShell(lane, inner) {
    ...
    return `<button class="nbc-btn nbp-back" type="button" data-panel="back">← Back to nudges</button>
      ...
      <div class="nbp-lanes">
        <button class="nbp-lane ${lane === "quick" ? "active" : ""}" type="button" data-panel="quick">Quick take</button>
        <button class="nbp-lane ${lane === "panel" ? "active" : ""}" type="button" data-panel="panel">Panel review</button>
      </div>
      <div class="nbp-body">${inner}</div>
  ...
  function renderPanel(lane) {
    ...
    if (lane === "quick") {
      ...
      el.innerHTML = panelShell("panel", `<div data-panel-seats><div class="nbp-quick"><span class="nbp-spin"></span>Convening the seats…</div></div><div class="nbp-agg nudg-hidden" data-panel-agg></div><div class="nbp-receipt nudg-hidden" data-panel-receipt></div>`);
      runPanel();
    }
  }
  ...
  async function runQuick() {
    ...
    try {
      const res = await relay("/api/quick", { question: panelCard.question, patient: chartFacts(p), context: [] }, 150000);
    ...
    } catch (e) { /* live lane failed: scripted already on screen */ }
  }
  ...
  function paintSeats(mode, seats, receiptText) {
    ...
    seatsBox.insertAdjacentHTML("beforebegin", `<span class="nb-mode ${cls}">${label}</span>`);
    ...
    if (agg) {
      ...
      agg.innerHTML = requests > 0 && support < Math.ceil(seats.length / 2)
        ? `UNDERDETERMINED <small>· the panel declines to ratify and lists what it needs first</small>`
        : `Panel support: ${support}/${seats.length} <small>· ${requests} requests data · dissent stays visible · support, never confidence</small>`;
    }
  ...
  async function runPanel() {
    ...
    if (!h.codex) return;
    try {
      const res = await relay("/api/panel", { question: panelCard.question, patient: chartFacts(p), seats: defaultSeats() }, 180000);
    ...
    } catch (e) { /* live run failed: scripted stays on screen */ }
  }
  ...
  NudgBus.on((evt) => {
    ...
    switch (evt.type) {
      ...
      case "nudge_superseded":
        if (evt.app !== "buddy" || !cards.has(d.id)) break;
        // Mirror outcomes that happened in the other tab.
        if (document.hidden || true) { cards.delete(d.id); if (panelCard && panelCard.id === d.id) exitPanel(); renderCards(); hidePeekFor(d.id); }
        break;
      case "demo_reset":
        cards.clear();
        noteSignals.clear();
