...
(function () {
  ...
  /* ---------------- config ---------------- */
  const DEMO = localStorage.getItem("nudg_demo_mode2") !== "0"; // demo-paced by default for the event
  ...
  const PENDING_EHR_COMMAND_KEY = "nudg_pending_ehr_command2";
  ...
  const REFERRAL_STATE_KEY = "nudg_referral_drafts2";
  ...
  function startCooldown(id) {
    ...
    save("nudg_cooldowns2", c);
  }
  function bumpMetric(key) {
    const m = store("nudg_metrics2", { evaluated: 0, shown: 0 });
    m[key] += 1;
    save("nudg_metrics2", m);
    return m;
  }
  ...
  function openEhrTarget() {
    try {
      const handle = window.open("", "nudg-ehr2");
      if (!handle) return null;
      let onEhrPath = false;
      let ready = false;
      let instanceId = null;
      try {
        onEhrPath = handle.location.href !== "about:blank" && handle.location.pathname.startsWith("/ehr2/");
    ...
    if (actionId === "open_rhythm") {
      ...
      if (APP !== "ehr") {
        if (target && target.existing) {
          ...
          if (target.needsNavigation) target.handle.location = "/ehr2/";
          target.handle.focus();
          jumped = true;
        } else {
  ...
  NudgBus.on((evt) => {
    ...
    switch (evt.type) {
      ...
      case "demo_reset":
        ...
        try { localStorage.removeItem(PENDING_EHR_COMMAND_KEY); } catch (e) { /* optional storage */ }
        activeMrn = null;
        save("nudg_cooldowns2", {});
        save("nudg_claims2", {});
        save("nudg_metrics2", { evaluated: 0, shown: 0 });
